...
#获取要获取的数据库
def getdatabases():
    dabasedb = MySQLdb.connect(host="52.220.234.108", user="root", db="station", passwd="wwwcodcom",port=3306, charset='utf8')
    # 获取还未爬取的页面url
    select_sql = """select id,domain,db_name,db_port,db_username,db_pass,ip from opencart_server_info where status=1
                    """
    databases=()
    try:
        cursor = dabasedb.cursor()
        cursor.execute(select_sql)
        databases = cursor.fetchall()

    except:
        print("Error: unable to fecth databases")
    finally:
        cursor.close()
    return databases
...
#更新获取数据库状态
def updatedatabasestatus(status,id):
    db = MySQLdb.connect(host="52.220.234.108", user="root", db="station", passwd="wwwcodcom", port=3306,
                               charset='utf8')
    select_sql = '''
                   update opencart_server_info set status=%s where id=%s
                    '''%(status,id)
    # print(select_sql)
    try:
        cursor = db.cursor()
        cursor.execute(select_sql)
        db.commit()
        print('update databasestatus=%s,id=%s successed!'%(status,id))
    except:
...
#发送产品数据到api接口
def sendcontent(singledeal,domain,db):
    try:
        ...
        re=requests.Session()
        url='http://'+domain+'/index.php?route=api/toadd&api_token'
        print("send url:"+url)
        result=re.post(url,data=datadict)
        print(result.text)
        print(result.status_code)
        if result.status_code==200:
            results = json.loads(result.text)
            updatestatus(1, urldict['id'],db)  # 获取商品详情成功
    except Exception as e:
        updatestatus(2, urldict['id'],db)  # 获取商品详情失败



while(True):
    print("start get databases")
    dabases=getdatabases()
    for a in dabases:
        try:
            print(a)
            id=a[0]
            ...
            password = a[5] #密码
            ip=a[6] #ip
            cc=dict(host = ip, user = username, db = dabaname, passwd = password, port = port, charset = 'utf8')
            # print(cc)
            db = MySQLdb.connect(**cc)
            # 模拟登陆
            # print("start login..")
            # dologinwait()
            # 获取要抓取的页面url
            ...
            for urldict in urls:
                ...
                try:
                    ...
                    print(e)
                    updatestatus(2,urldict['id'],db)  # 将状态修改为失败状态2
                    continue
                ...
                if len(result)!=0:
                    ...
                    sendcontent(c,domain,db)
                else:
                    updatestatus(2,urldict['id'],db)   #将状态修改为失败状态2
                print('sleep 3s...\n')
                time.sleep(5)   #睡眠5秒防止太频繁被封

            updatedatabasestatus(0,id)# 更新爬取状态
